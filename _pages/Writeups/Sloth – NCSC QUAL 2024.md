...
The primes `p` and `q` are generated as $p = 2 \cdot \prod_{i=0}^{9} p_i + 1$ and $q = 2 \cdot \prod_{i=0}^{9} q_i + 1$, where each $p_i$ and $q_i$ is randomly chosen from the prime list.
